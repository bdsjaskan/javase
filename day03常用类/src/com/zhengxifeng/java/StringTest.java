...
public class StringTest {
    ...
    public void test3(){
        ...
        String s8 = s5.intern();//返回值得到的s8是使用的常量池中已经存在的“javaEEhadoop”
    ...
    public void test2(){
        ...
        String s1 = "javaEE";
        String s2 = "javaEE";
        //通过 new + 构造器 的方式：此时s3 和 s4保存的地址值，是数据在堆空间中开辟空间以后对应的地址值
    ...
    4.String：代表不可变的字符序列。简称：不可变的特性。
           体现：1.当对字符串重新赋值时，需要重写指定内存区域赋值，不能使用原有的value进行赋值
                ...
                3.当调用String的replace()方法修改指定的字符或字符串也必须重新指定区域赋值，不能使用原有的value进行赋值
    5.通过字面量的方式（区别于new的方式）给一
    6.字个字符串赋值，此时的字符串值声明在字符串常量池中。
