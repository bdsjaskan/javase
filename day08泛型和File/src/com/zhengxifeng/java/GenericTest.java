 ...
 * ③ 指明完以后，再集合类或接口中定义类或接口时，内部结构（比如 ：方法、构造器、属性等） 使用到类
 *      比如：add(E e) ---> 实例化以后: add(Integer e)
 * ④ 注意点：泛型的类型必须是类，不能是基本数据类型。需要用到基本数据类型的位置，用相应的包装类替换
 * ⑤ 如果实例化时，没有指明泛型的类型。 那么默认类型为 java.long.Object 类型
 *
 * 3.如何自定义泛型结构：泛型类  、泛型接口； 泛型方法：   见 GenericTest1
 *
 * @author shkstart
 * @create 2022-01-23 19:06
 */
