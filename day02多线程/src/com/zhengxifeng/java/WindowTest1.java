 ...
 *
 * 1.问题：卖票的过程中，出现了错票、重票-->出现了线程的安全问题
 * 2.问题出现的原因：当某个线程操作车票的过程中，尚未操作完时，其他线程参与进来，也操作车票。
 * 3.如何解决：当一个线程a在操作ticket的时候，其他线程不能参与进来。 知道a线程操作完ticket时，
 *            其他线程才可以开始操作ticket。这种情况即使线程a出现了阻塞，也不能被改变。
 *
 * 4.java当中，我们通过同步机制，来解决线程的安全问题。
 *    方式一：同步代码块
 *
 *     synchronized(同步监视器){
 *         //需要被同步的代码
 ...
 *            3.同步监视器，俗称：锁。任何一个类的对象，都可以充当这个锁
 *                  要求：多个线程必须共用同一把锁。
...
//    Dog dog = new Dog();
    ...
    public void run() {
        ...
        while (true) {
            ...
            synchronized (this) {//唯一对象window1   //synchronized (obj) {
